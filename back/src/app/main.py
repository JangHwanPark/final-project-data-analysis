...
def run_pipeline():
  ...
  logger.info("==============================================")
  logger.info(" PYTHON DATA ANALYSIS PIPELINE STARTED (Ver 3.0)")
  logger.info("==============================================")
  ...
  try:
    ...
    steps.logger.info(f"Writing public JSON artifact to {FRONTEND_PUBLIC_DIR.name}...")
    artifact_writer.write_analysis_json(summary, FRONTEND_PUBLIC_DIR)

    # [JSON 저장] 프론트엔드 Shared Data 디렉토리용
    steps.logger.info(f"Writing public JSON artifact to {FRONTEND_SHARED_DIR.name}...")
    artifact_writer.write_analysis_json(summary, FRONTEND_SHARED_DIR)
    ...
    # reporter = _get_excel_reporter()
    # excel_path = reporter.write_report(summary)

    logger.info("\n\n")
    logger.info("==============================================")
    logger.info("IPELINE EXECUTION COMPLETE. (Artifacts Summary)")
    logger.info(f" Public JSON saved to: {FRONTEND_PUBLIC_DIR}")
    logger.info(f" Shared JSON saved to: {FRONTEND_SHARED_DIR}")
